...
async function init(){
  ...
  document.title = `${p.name} | Diamond Sect`;
  const metaDesc = document.querySelector('meta[name="description"]');
  if(metaDesc && p.shortDescription) metaDesc.setAttribute("content", p.shortDescription);
  ...
  wrap.innerHTML = `
    ...
    <div class="detail-layout">
      <div>
        ...
        <div class="thumb-row">
          ${images.map((img,i)=>`<div class="thumb ${i===0?'active':''}" data-img="${img}"><img src="${img}" alt="${p.name} - foto ${i+1}"></div>`).join("")}
        </div>
      </div>
