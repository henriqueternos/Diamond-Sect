...
import { updateCartCount, CART_KEY, showToast } from "./main.js";
...
async function renderGrid(container, data){
...
async function init(){
  const containers = document.querySelectorAll("[data-products-grid]");
  if(!containers.length) return;
  const data = await readPath("products/products/items");
  containers.forEach(container => renderGrid(container, data));
}
init();