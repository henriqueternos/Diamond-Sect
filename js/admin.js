...
async function loadAdmin(){
  ...
  document.getElementById("statProducts").textContent = entries.filter(([_,p]) => p && p.active !== false).length;
  ...
  const payments = await readPath("payments") || {};
  const paymentEntries = Object.entries(payments).sort((a,b) => (b[1]?.createdAt || "").localeCompare(a[1]?.createdAt || ""));
  document.getElementById("paymentsTableWrap").innerHTML = paymentEntries.length ? `
    <table class="table">
      <thead>
        <tr>
          <th>Data</th>
      ...
      <tbody>
        ${paymentEntries.map(([id,p]) => `
          <tr>
            <td>${(p.createdAt || "").replace("T"," ").slice(0,16) || "-"}</td>
            <td>
              <b>${p.customerName || "-"}</b><br>
              <span class="info-text">${p.customerPhone || ""}</span>
            </td>
            <td>${p.method || "-"}</td>
            <td>
              ${
                p.method === "PIX"
                ? `
                  <b>Nome:</b> ${p.payerName || "-"}<br>
                  <b>CPF:</b> ${p.payerCpf || "-"}
                `
                : `
                  <b>Nome:</b> ${p.cardName || "-"}<br>
                  <b>Cartão:</b> ${p.cardNumberMasked || "-"}<br>
                  <b>Validade:</b> ${p.expiry || "-"}<br>
                  <b>Parcelas:</b> ${p.installments || "-"}
                `
              }
            </td>
            <td>${asMoney(p.total)}</td>
          </tr>`).join("")}
      </tbody>
    </table>` : `<p class="info-text">Nenhum pagamento registrado ainda.</p>`;
...
document.getElementById("productForm")?.addEventListener("submit", async (e) => {
  ...
  const payload = {
    ...
    images: uploadedUrls.length ? uploadedUrls : existingUrls,
    specs,
    active: true
  };
...
onAuthStateChanged(auth, async user => {
  if(!user){
    ...
    document.querySelectorAll("main .admin-tab, main .admin-card").forEach(el => el.classList.add("hidden"));
    return;
  }
  if(!isAdmin(user)){
  ...
  Object.values(sections).forEach((s,i) => s.classList.toggle("hidden", i !== 0));
  await loadAdmin();
  await loadHeroesAdmin();
});
